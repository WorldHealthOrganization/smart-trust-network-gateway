...
---
> The architecture documentation of the Trust Network Gateway will be continued within the scope of  [smart-trust repository](https://github.com/WorldHealthOrganization/smart-trust).
---
# Terminology
...
| NB<sub></sub>             | The certificate that a national backend uses to sign data packages that are uploaded to the TNG.                                                                                                                                                                                                                |
...
In the Bilateral Onboarding use case, the [connector library](https://github.com/WorldHealthOrganization/ddcc-gateway-lib) connects as a trusted party to multiple gateways for downloading/uploading content. This requires onboarding on both gateways. 
...
<p align="center">
  <img src="pictures/architecture/PrimarySecondaryExchange.drawio.png" alt="TNG Use Case - Primary-Secondary Exchange" style="width:400px;"/>
</p>
...
<p align="center">
  <img src="pictures/architecture/CombineSourcesExchange.drawio.png" alt="TNG Use Case - Primary-Secondary" style="width:400px;"/>
</p>
...
The Trust Network Gateway's purpose is to enable the secure and trusted exchange of data within a trust network.
...
The DDCC specification provides interoperable standards for exchanging metadata content such as trusted references, trusted certificates and signer certificates with systems via a Trust Network Gateway. This metadata is managed through Trusted Systems which will need a connection/proxying or facade service with the Trust Network Gateway (“TNG Mediator”). This mediator must be onboarded and trusted by the operator of the TNG before an up/download of content is possible. Technically this can be a script, a backend system or an OpenHIM mediator. The main tasks of this kind of software is to establish an mTLS connection with the gateway, sign the uploaded content (e.g. CMS Cryptographic Message Syntax) and upload signed DSCs, revocation entries or releasing business rules. The procedures used in background are out of scope. There may be manual release processes, automatic decisions or other processes, however it is crucial to ensure that the trusted channel and the security of the certificates used for upload/TLS connections are not compromised.

### Options for Bridging to other Systems
A bridge tool for translating the received entries of the originating system to the HL7 FHIR / Rest API of the gateway is necessary for bridging existing systems (i.e. PKDs or any other systems which contain PKI certificates (e.g. ICAO), Business Rules or Value Sets (e.g. FHIR Servers)) to the TNG. For example, to translate an SQL-based Public Key directory to the gateway, a script/mediator may be set up to extract the DSCs and upload them automatically to the gateway. Please note that in this scenario all CSCAs must be onboarded prior to the upload for this process to work. 

Under special circumstances when some “mass data transactions” or heavy synchronizations are necessary there may be an option to set up an adapter directly on top of the gateway database. The TNG itself supports JDBC which is able to accept other databases than MySQL. For instance if a Cassandra, MongoDb or CouchDB is used and a JDBC driver is available, the data can be replicated across multiple nodes. 

<b>Note</b>: Database Replications have their own behavior and the functionality of the gateway cannot cover each available database. Therefore the JDBC feature should be used only if necessary and at one's own risk.
...
* Raw key cannot be verified by the source (e.g. Root Authority)
...
The usage of the OID can limit the scope of a Document Signer Certificate during the verification process (if supported by the verifier app). For instance, fraudulent vaccination certificates issued by test centers will not be valid, as they are signed by a DSC limited to test result certificate issuers. 
...
|Verb|Used in Federator|Used in Mediator|Route|Modification/Behavior|
|----|-----------------|---------------|-----|---------------------|
|GET|X|X|/trustlist/certificates|Returns the list of trusted certificates. The list can be filtered with optional query parameters. For legacy reasons, all signer certificates are introduced in the “DSC” certificate group (if profile enabled). All other certificates should be delivered via query parameter.|
...
Each route which delivers federated data must provide a query parameter to filter the federations by using an array:
<p align="center"> 
    /URL?federationId=id1,id2,id3&Domain=DCC&ResourceType=...}
</p>
When the mode is set to “GATEWAYONLY”, it must be used to query just for the configured gateway ID.
...
To federate multiple gateway data, a download process is introduced which should ensure that only trusted data is downloaded to a local gateway. Trusted data means in this context that the operator of a local gateway has the total control which federated data is accepted and which not. To achieve this target, the local gateway operator must explicitly onboard any remote federators plus the trust anchors of the data which can be accepted. This is necessary because each remote federator may deliver the data of multiple other gateways (which are trusted by the origin gateway operator), but this means not necessarily that this data is trusted automatically by the local gateway operator as well (implicit trust relations must be avoided). Therefore, during the download process, a check should be run which skips all data that is not explicitly trusted by the local operator. This can be achieved by whitelisting multiple trust anchors and the cross check via the NBUP certificates. If the trust chain is established in this way, every content can be downloaded, verified and pushed to the store. The entire download process itself follows a delta download mechanism, which every day downloads the entire content and within the day just the deltas. This means for the trust network that a certificate “bubbles” from the origin gateway step by step to all other gateways. Because of this behavior, one must consider a delay of around one day between creating a key pair and issuing the first certificates with it.
...
### Data Tables
The trusted party table (see chapter 4.2.3.1, EU DCC Gateway) is enhanced with a new certificate type “TRUSTANCHOR”. In the API call for trust lists these new types appear. To distinguish between a federator and a normal trusted party, a type (“TP”, “FEDERATOR”,”GATEWAY”)  for the trusted item is introduced. To distinguish between different domains of certificates, the table also gets a new column ''DOMAIN”, which has the default content “DCC''. As of now, other content can be “ICAO”, “DIVOC” and “SHC”. The domain appears in the trustlist routes.
...
### Route Profiles
The routes for POST, PUT and DELETE will be modified by profiles to make them configurable. This allows it to switch off the data upload, which especially serves the primary-secondary/combined sources use case. Within this setup, no NBUP certificates need to be onboarded.
...
Note: This new route is introduced to create a migration path to the trusted references. Within EU DCC Standard Mode, there is no backward compatibility impact. 

### Trusted References
The trusted references are URLs which are uploaded by the member states to propagate their service endpoint about value sets, business rules and other content for interoperability. Within the trusted references are only public GET methods are allowed. Authorization must be covered by trust mediators, if necessary.   
...
<b>Note</b>: When the URL in this table does not resolve, all the optional fields can be empty. This is less trustworthy and should be avoided within operations.  

### Deployment
#### Constraints
The TNG may be operated with a network component (Load Balancer, API Gateway, Reverse Proxy etc.) in front which handles the Client Certificate Authentication and Client Certificate Attribute extraction of the TLS connection. After the TLS Offloading it depends on the infrastructure whether an internal secure TLS network must be established or not. For example when the TNG is deployed in a distributed service mesh, it is recommended to use TLS-protected channels e.g. SPIFFE/SPIRE-based service meshes. Which mode fits better to the deployment depends on the operator's infrastructure. The gateway itself can be operated in SSL Passthrough mode as well.
...
To support the DID document feature according to the [trust specification](https://github.com/WorldHealthOrganization/ddcc-trust/blob/main/TrustListSpecification.md#leading-contender-did-document), the gateway will be enhanced by a DID document generator and the capability to sign these DID documents. This contains a way to generate signing keys via a vault and register the public key of the signature via a decentralized key registry.

![](pictures/architecture/TechnicalChanges.drawio.png)

### Azure Architecture & Deployment

Microsoft Azure public cloud is the target operation environment for the Trust Network Gatewa hosted by WHO. The TNG will be operated on Azure Kubernetes Service (AKS) and uses Azure CDN for public key distribution via DID documents.
...
The WHO Trust Network Secretariat is responsible for managing the onboarding application of member states to be connted as a trusted party to the trust network. Prepared onboarding records will be handed over to the TNG operator with the request to process the technical onboarding of the trusted party. An organizationally and technically successful application results in a confirmation and the member state can connect to the network as a trusted party. 
...
* Providing a Secure Channel for the member states to deliver secure and trustworthy applications and DID/CSCA information
* Creating and Securing a Key Pair (Trust Anchor) to sign/confirm onboarding requests for the gateway
